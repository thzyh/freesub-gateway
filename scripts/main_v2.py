...
GATEWAY_CANDIDATES_PATH = os.path.join(OUTPUT_DIR, "gateway-candidates.json")

# URI -> all subscription URLs that supplied it during this run. The old
# pipeline used a set and consequently lost provenance before probing.
SOURCE_MAP = {}
    ...
    with ThreadPoolExecutor(MAX_WORKERS_FETCH) as ex:
        ...
        for f in as_completed(futs):
            ...
            if err:
                ...
                for uri in got:
                    SOURCE_MAP.setdefault(uri, set()).add(url)
            nodes.update(got)
...
def classify_and_export(test_results: list):
    ...
    for r in test_results:
        ...
        nodes.append({
            ...
            "upstream_sources": sorted(SOURCE_MAP.get(r["raw"], set())),
            "tested_at": r.get("tested_at") or datetime.now(timezone.utc).isoformat(),
        })
...
def gateway_candidate_id(item):
    """Return a stable, non-secret identifier for a parsed node."""
    outbound = dict(item.get("outbound") or {})
...
def export_gateway_candidates(nodes):
    """Write a small, provenance-preserving feed for the Gateway.

    This is intentionally separate from the user subscription formats. The
    feed is generated by Actions, while nyVPS performs its own final probe
    before starting any runtime process.
    """
    candidates_by_id = {}
    for item in nodes:
        ...
        if existing is None:
            ...
            continue
        sources = sorted(set(existing["upstream_sources"] + candidate["upstream_sources"]))
        existing_rank = (existing["risk_score"], existing["latency_ms"] or 10**9, -existing["speed_bps"])
        candidate_rank = (candidate["risk_score"], candidate["latency_ms"] or 10**9, -candidate["speed_bps"])
        if candidate_rank < existing_rank:
            candidates_by_id[candidate_id] = candidate
        candidates_by_id[candidate_id]["upstream_sources"] = sources
    ...
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    fd, tmp_path = tempfile.mkstemp(prefix="gateway-candidates.", suffix=".tmp", dir=OUTPUT_DIR)
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as stream:
            json.dump(payload, stream, indent=2, ensure_ascii=False)
            stream.write("\n")
        os.replace(tmp_path, GATEWAY_CANDIDATES_PATH)
    finally:
...
def export_all(unique_nodes, residential, non_residential):
    ...
    gateway_count = export_gateway_candidates(unique_nodes)
    print(f"[*] 导出完毕: 全量 {len(all_links)} | 家宽 {len(res_links)} | Gateway 候选 {gateway_count}")
